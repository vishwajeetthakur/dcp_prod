...
@router.get("/devices/{device_id}/details", response_model=AllDeviceDataResponse)
async def get_device_info_by_devicekey(
    device_id: str, connection: Session = Depends(get_db)
):
    stmt = (
        select(
        ...
        .filter(DeviceTable.device_key == device_id)
