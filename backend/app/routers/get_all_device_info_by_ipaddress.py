...
@router.get("/devices/ip/{ip_address}/details", response_model=AllDeviceDataResponse)
async def get_device_info_by_ipaddress(
    ip_address: IPv4Address, connection: AsyncSession = Depends(get_db)
):
    stmt = (
        select(
        ...
        .filter(DeviceTable.ip_address_v4 == ip_address)
    ...
    return AllDeviceDataResponse.from_orm(dict(device_data))
